...
class PropertyAdmin(admin.ModelAdmin):
    list_display    = ('thumb', 'title', 'property_type', 'purpose',
                       'formatted_price_display', 'city', 'status',
                       'is_featured', 'is_verified', 'views_count', 'created_at')
...
class InquiryAdmin(admin.ModelAdmin):
    ...
    def property_link(self, obj):
        return format_html(
            ...
            obj.property.pk, obj.property.title[:50]
        )
...
class PropertyEnlistRequestAdmin(admin.ModelAdmin):
    ...
    # ← removed list_editable on status to avoid conflicts with actions
    ...
    fieldsets = (
        ...
        ('Property Details', {
            'fields': ('property_type', 'purpose', 'city', 'locality',
    ...
    def approve_and_publish(self, request, queryset):
        """
        Approves selected enlist requests AND automatically creates
        a live Property listing from each one.
        """
        created_count  = 0
        skipped_count  = 0
        ...
        for enlist in queryset:
            ...
            if enlist.bedrooms:
                title = (
                    f"{enlist.bedrooms} BHK "
                    f"{enlist.get_property_type_display()} in "
                    f"{enlist.locality}, {enlist.city}"
                )
            else:
                title = (
                    f"{enlist.get_property_type_display()} in "
            ...
            # Create the live Property object
            prop = Property(
                title         = title,
                description   = enlist.description,
                property_type = enlist.property_type,
                purpose       = enlist.purpose,
                address       = f"{enlist.locality}, {enlist.city}",
            ...
            prop.save()  # slug is auto-generated inside model.save()

            # Mark enlist request as approved
        ...
        msg = f"✅ {created_count} listing(s) published successfully!"
        if skipped_count:
            msg += f" ({skipped_count} already approved, skipped)"
        self.message_user(request, msg)

    approve_and_publish.short_description = "✅ Approve & publish as live listing"
